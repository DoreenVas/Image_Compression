...
if __name__ == "__main__":
    A, img_size = read_file()
    height = img_size[0]
    width = img_size[1]
    pixel_vec_size = img_size[2]
    ...
    while k <= 16:
        ...
        # ten iterations for each k
        #loss = []
        for i in range(10):
            #temp = 0
            new_centroid_sum = np.zeros((k, pixel_vec_size))
            new_centroid_mone = np.zeros(k)
            for j in range(height):
                for p in range(width):
                    ...
                    for c in range(k):
                        dis_vec = A[j][p] - centroids[c]
                        # euclidean distance squared
                        euc_dis_vec.append(np.linalg.norm(dis_vec) ** 2)
                    centroid = np.argmin(euc_dis_vec)
                    #temp += euc_dis_vec[centroid]
                    new_centroid_sum[centroid] += (A[j][p])
                    new_centroid_mone[centroid] += 1
            #loss.append(temp/(height*width))
            # find new centroid (average)
            for cen in range(k):
                centroids[cen] = new_centroid_sum[cen] / new_centroid_mone[cen]
            print("iter "+str(i+1)+": "+print_cent(centroids))
        """
        # change A and show the new image
        for j in range(height):
            for p in range(width):
                euc_dis_vec = []
                for c in range(k):
                    dis_vec = [A[j][p][0] - centroids[c][0], A[j][p][1] - centroids[c][1], A[j][p][2] - centroids[c][2]]
                    euc_dis_vec.append(np.linalg.norm(dis_vec) ** 2)
                centroid = np.argmin(euc_dis_vec)
                A[j][p] = centroids[centroid]
        plt.imshow(A)
        plt.grid(False)
        plt.show()
